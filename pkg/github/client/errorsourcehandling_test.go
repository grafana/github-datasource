...
func TestAddErrorSourceToError(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "bad credentials error message",
			err:      errors.New("Bad credentials"),
			resp:     nil,
			expected: backend.DownstreamError(errors.New("Bad credentials")),
		},
		{
			name:     "service unavailable error message",
			err:      errors.New("Service Unavailable"),
			resp:     nil,
			expected: backend.DownstreamError(errors.New("Service Unavailable")),
		},
		{
			// Regression: go-github's Response embeds a *http.Response that can be
			// nil, even when the Response wrapper is not nil. Then resp.StatusCode
			// panics. The function must return the error as-is. It must not crash.
